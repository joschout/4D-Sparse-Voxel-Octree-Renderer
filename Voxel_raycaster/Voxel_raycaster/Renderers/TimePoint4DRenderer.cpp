...
void TimePoint4DRenderer::Render(RenderContext const& rc, Tree4D const* tree, unsigned char* texture_array, float time_point) const
{
...
#pragma omp parallel for private(x,t,v,index,factor)
	for (int y = 0; y < rc.n_y; y++) {
		...
		for (x = 0; x < rc.n_y; x++) {
			...
			float nbOfTimeSteps = 0.0;
			for (float timestep = 0.0f; timestep < endtime; timestep = timestep + unitlength_time) {
