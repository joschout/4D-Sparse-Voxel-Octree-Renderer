...
void DiffuseTree4DRenderer::calculateAndStoreColorForThisPixel(RenderContext const& rc, Tree4D const* tree, unsigned char* texture_array, int index, Tree4DTraverserDifferentSides &treeTraverser) const
{
	...
	for (int i = 0; i< rc.lights.size(); i++) {
		if (rc.lights[i].active) {
			vec3 to_light = rc.lights[i].position - treeTraverser.getCurrentPosition();
			vec3 s = to_light + (rc.camera->eye - treeTraverser.getCurrentPosition());
			s = normalize(s);

			float distancecut = 1.0f / (
			...
			// Diffuse
			float diffuse_factor = tree->data[treeTraverser.getCurrentNode()->data].normal DOT to_light;
			...
			// Phong
			float phong_factor = pow(to_light DOT tree->data[treeTraverser.getCurrentNode()->data].normal, rc.lights[i].SHININESS);
